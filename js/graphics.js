...
/**
 * Определяет текущую тему на основе WebApp (МАКС) или системной темы устройства.
 * Возвращает 'dark' или 'light'.
 */
export function detectTheme() {
    // 1. Если запущено внутри МАКС — берём тему из мессенджера
    if (window.WebApp && typeof window.WebApp.colorScheme === 'string') {
        return window.WebApp.colorScheme === 'dark' ? 'dark' : 'light';
    }
    // 2. Если WebApp нет (запуск на компе/в обычном браузере) —
    //    по ТЗ тема по умолчанию светлая.
    //    Если захочешь, чтобы на компе тоже реагировало на системную тему ОС —
...
/**
 * Применяет тему к body через класс .dark-theme.
 */
